...
- Contributions containing AI-generated content must be disclosed.
    - Commits must include an `Assisted-by` tag naming the tool used. Examples: `Assisted-by: Claude`, `Assisted-by: OpenAI Codex`.
    - Commits must not use a `Co-Authored-by` tag for AI attribution. Only humans can be co-authors.
